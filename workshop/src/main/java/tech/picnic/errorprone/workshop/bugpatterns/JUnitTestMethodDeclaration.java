...
public final class JUnitTestMethodDeclaration extends BugChecker implements MethodTreeMatcher {
  ...
  public Description matchMethod(MethodTree tree, VisitorState state) {
    // XXX: Part 1: Return `Description.NO_MATCH` if the method is not a {@link TEST_METHOD}`.

    SuggestedFix.Builder fixBuilder = SuggestedFix.builder();

    // XXX: Part 2: Ensure JUnit test methods don't use {@link ILLEGAL_MODIFIERS} by using
